#include "../../headers/commands/UnlikeQuizCommand.h"
#include "../../headers/services/QuizService.h"
#include "../../headers/helpers/Validate.h"

UnlikeQuizCommand::UnlikeQuizCommand(const MyString& buffer, Context& ctx) : Command(buffer, ctx){
...
void UnlikeQuizCommand::execute(){
	...
	if (!QuizService::isQuizLiked(ctx, quiz)){
		std::cout << NotLiked << std::endl;
		return;
	}

	QuizService::unlikeQuiz(ctx, quiz);
}
